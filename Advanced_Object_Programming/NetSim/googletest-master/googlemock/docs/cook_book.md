...
class MockFoo : public Foo {
  ...
  using Foo::Add;
  MOCK_METHOD(int, Add, (Element x), (override));
  // We don't want to mocks int Add(int times, Element x);
  ...
};
```
...
// A mocks packet stream class.  It inherits from no other, but defines
...
class MockFoo : public Foo {
 public:
  // Normal mocks method definitions using gMock.
 ...
 private:
  FakeFoo fake_;  // Keeps an instance of the fake in the mocks.
};
```
...
class MockFoo : public Foo {
 public:
  ...
  // where this mocks class is used.
...
// variables used to implement the mocks methods.
MockFoo::MockFoo() {}
MockFoo::~MockFoo() {}
```
...
TEST(FooTest, InvokesBarCorrectly) {
  MyMock mock;
  // Class MockFunction<F> has exactly one mocks method.  It is named
  // Call() and has type F.
...
class MockFoo : public Foo {
  ...
  // Add the following two lines to the mocks class.
...
```cpp
// DuplicateArg<k, T>(output) converts the k-th argument of the mocks
