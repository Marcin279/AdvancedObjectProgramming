...
void perform_report_check(std::function<void(std::ostringstream&)>& reporting_function,
    ...
    for (auto& lines : expected_report_lines) {
        expected_report_oss << lines << "\n";
    }
...
TEST(SpecificTurnsReportNotifierTest, should_generate_raport) {
    /*
     * Zwraca True jeśli parametr t funkcji should_generate_raport należy do zbioru
     * zawartości numerów tur St
     */

    SpecificTurnsReportNotifier spec(std::set<Time>{1, 3, 6});
    bool should_be_generated = spec.should_generate_report(1);
    bool should_not_to_be_generated = spec.should_generate_report(2);

    ASSERT_TRUE(should_be_generated);
    ASSERT_FALSE(should_not_to_be_generated);
}


TEST(IntervalReportNotifierTest, should_generate_rapor) {
    /*
     * Na podstawie interwału czasowego delta_t przekazanego w konstruktorze należy wygenerować raport do delta_t tur
     * począwszy od 1.
     * czyli dla delta_t = 2 raporty powinny być wygenerowane w turach 1, 3, 5, 7, ...
     */
    IntervalReportNotifier interv_notifier(2);
    bool should_generate_raport1 = interv_notifier.should_generate_report(1);
    bool should_generate_raport3 = interv_notifier.should_generate_report(3);
    bool should_not_generate_raport4 = interv_notifier.should_generate_report(4);
    bool should_generate_raport5 = interv_notifier.should_generate_report(5);

    ASSERT_TRUE(should_generate_raport1);
    ASSERT_TRUE(should_generate_raport3);
    ASSERT_TRUE(should_generate_raport5);
    ASSERT_FALSE(should_not_generate_raport4);
}